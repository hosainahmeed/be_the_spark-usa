import Link from 'next/link';
import React from 'react';

function SocialIcons() {
    return (
        <div className="flex items-center justify-center gap-4">
            <Link href="https://www.facebook.com/">
                <div className="hover:cursor-pointer">
                    <svg
                        width="28"
                        height="28"
                        viewBox="0 0 28 28"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                    >
                        <path
                            d="M23.8984 0H4.10156C1.84012 0 0 1.84012 0 4.10156V23.8984C0 26.1599 1.84012 28 4.10156 28H12.3594V18.1016H9.07812V13.1797H12.3594V9.84375C12.3594 7.12966 14.5672 4.92188 17.2812 4.92188H22.2578V9.84375H17.2812V13.1797H22.2578L21.4375 18.1016H17.2812V28H23.8984C26.1599 28 28 26.1599 28 23.8984V4.10156C28 1.84012 26.1599 0 23.8984 0Z"
                            fill="white"
                        />
                    </svg>
                </div>
            </Link>
            <Link href="https://www.twitter.com/">
                <div className="hover:cursor-pointer">
                    <svg
                        width="28"
                        height="28"
                        viewBox="0 0 28 28"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                    >
                        <path
                            d="M14.9361 13.522L21.2371 22.5347H18.6511L13.5094 15.1803V15.1798L12.7545 14.1002L6.74805 5.50854H9.334L14.1812 12.4424L14.9361 13.522Z"
                            fill="white"
                        />
                        <path
                            d="M24.9745 0H3.02546C1.35459 0 0 1.35459 0 3.02546V24.9745C0 26.6454 1.35459 28 3.02546 28H24.9745C26.6454 28 28 26.6454 28 24.9745V3.02546C28 1.35459 26.6454 0 24.9745 0ZM17.8593 23.7445L12.6561 16.172L6.14174 23.7445H4.45809L11.9086 15.0844L4.45809 4.24108H10.1407L15.0677 11.4118L21.2364 4.24108H22.9201L15.8155 12.4996H15.8151L23.5419 23.7445H17.8593Z"
                            fill="white"
                        />
                    </svg>
                </div>
            </Link>
            <Link href="https://www.linkedin.com/">
                <div className="hover:cursor-pointer">
                    <svg
                        width="28"
                        height="28"
                        viewBox="0 0 28 28"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                    >
                        <path
                            d="M25.2 0H2.8C1.26 0 0 1.26 0 2.8V25.2C0 26.74 1.26 28 2.8 28H25.2C26.74 28 28 26.74 28 25.2V2.8C28 1.26 26.74 0 25.2 0ZM8.4 23.8H4.2V11.2H8.4V23.8ZM6.3 8.82C4.9 8.82 3.78 7.7 3.78 6.3C3.78 4.9 4.9 3.78 6.3 3.78C7.7 3.78 8.82 4.9 8.82 6.3C8.82 7.7 7.7 8.82 6.3 8.82ZM23.8 23.8H19.6V16.38C19.6 15.2601 18.62 14.28 17.5 14.28C16.38 14.28 15.4 15.2601 15.4 16.38V23.8H11.2V11.2H15.4V12.88C16.1 11.76 17.64 10.92 18.9 10.92C21.56 10.92 23.8 13.16 23.8 15.82V23.8Z"
                            fill="white"
                        />
                    </svg>
                </div>
            </Link>
            <Link href="https://www.youtube.com/">
                <div className="hover:cursor-pointer">
                    <svg
                        width="28"
                        height="28"
                        viewBox="0 0 28 28"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                    >
                        <path
                            d="M14.0004 8.86475C11.1668 8.86475 8.86523 11.1663 8.86523 13.9999C8.86523 16.8335 11.1668 19.1407 14.0004 19.1407C16.834 19.1407 19.1412 16.8335 19.1412 13.9999C19.1412 11.1663 16.834 8.86475 14.0004 8.86475Z"
                            fill="white"
                        />
                        <path
                            d="M21.7504 0H6.2496C2.8056 0 0 2.8056 0 6.2496V21.7504C0 25.2 2.8056 28 6.2496 28H21.7504C25.2 28 28 25.2 28 21.7504V6.2496C28 2.8056 25.2 0 21.7504 0ZM14 23.072C8.9992 23.072 4.928 19.0008 4.928 14C4.928 8.9992 8.9992 4.9336 14 4.9336C19.0008 4.9336 23.072 8.9992 23.072 14C23.072 19.0008 19.0008 23.072 14 23.072ZM23.2624 6.58C22.204 6.58 21.3416 5.7232 21.3416 4.6648C21.3416 3.6064 22.204 2.744 23.2624 2.744C24.3208 2.744 25.1832 3.6064 25.1832 4.6648C25.1832 5.7232 24.3208 6.58 23.2624 6.58Z"
                            fill="white"
                        />
                    </svg>
                </div>
            </Link>
            <Link href="https://www.instagram.com/">
                <div className="hover:cursor-pointer">
                    <svg
                        width="28"
                        height="28"
                        viewBox="0 0 28 28"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                    >
                        <path
                            d="M18.8412 9.15886L18.8062 9.12386C17.6621 7.97941 16.1472 7.28074 14.534 7.1535C12.9208 7.02626 11.3151 7.47881 10.0058 8.42974C8.69645 9.38068 7.76938 10.7676 7.39133 12.341C7.01328 13.9145 7.20905 15.5712 7.94344 17.0132L8.24253 17.6018L7.68571 20.3064L7.67776 20.3414L10.3648 19.705L10.9773 20.0152C12.2517 20.6483 13.6926 20.866 15.0971 20.6377C16.5017 20.4095 17.7994 19.7466 18.8078 18.7425C20.0761 17.474 20.7915 15.7556 20.798 13.9619C20.8046 12.1681 20.1018 10.4445 18.8428 9.16681L18.8412 9.15886ZM18.5675 16.7507C18.2382 17.2693 17.718 17.9025 17.0641 18.06C15.9187 18.3384 14.1607 18.0695 11.9732 16.0316L11.9462 16.0061C10.0228 14.2243 9.52321 12.74 9.64412 11.5627C9.71094 10.8929 10.2678 10.29 10.7371 9.89386C10.8114 9.83086 10.8994 9.78605 10.9941 9.763C11.0887 9.73995 11.1875 9.73928 11.2824 9.76106C11.3774 9.78283 11.466 9.82644 11.5412 9.88843C11.6163 9.95043 11.676 10.0291 11.7155 10.1182L12.4234 11.7186C12.4698 11.8217 12.487 11.9355 12.4733 12.0477C12.4596 12.1599 12.4154 12.2662 12.3455 12.355L11.9875 12.8195C11.9126 12.9148 11.8673 13.03 11.8574 13.1508C11.8475 13.2716 11.8734 13.3927 11.9319 13.4989C12.1323 13.8489 12.6128 14.3659 13.1457 14.8464C13.7439 15.3857 14.4073 15.8804 14.8273 16.0507C14.9399 16.0957 15.0634 16.1064 15.1821 16.0811C15.3008 16.0559 15.4093 15.9961 15.4939 15.9091L15.9091 15.4907C15.9892 15.4117 16.0889 15.3553 16.1979 15.3274C16.3069 15.2994 16.4213 15.3009 16.5296 15.3316L18.2112 15.8089C18.3042 15.8372 18.3894 15.8866 18.4604 15.953C18.5313 16.0195 18.5861 16.1013 18.6205 16.1923C18.655 16.2832 18.6681 16.3808 18.6589 16.4776C18.6497 16.5744 18.6184 16.6678 18.5675 16.7507Z"
                            fill="white"
                        />
                        <path
                            d="M24.1818 0H3.81818C2.80554 0 1.83437 0.402272 1.11832 1.11832C0.402272 1.83437 0 2.80554 0 3.81818L0 24.1818C0 25.1945 0.402272 26.1656 1.11832 26.8817C1.83437 27.5977 2.80554 28 3.81818 28H24.1818C25.1945 28 26.1656 27.5977 26.8817 26.8817C27.5977 26.1656 28 25.1945 28 24.1818V3.81818C28 2.80554 27.5977 1.83437 26.8817 1.11832C26.1656 0.402272 25.1945 0 24.1818 0ZM22.6752 14.2068C22.6283 15.6603 22.217 17.0786 21.4791 18.3317C20.7412 19.5848 19.7004 20.6325 18.4521 21.3785C17.2039 22.1246 15.7883 22.5452 14.3351 22.6016C12.882 22.658 11.438 22.3486 10.1357 21.7016L5.75273 22.7405C5.6863 22.7559 5.61674 22.751 5.55316 22.7263C5.48958 22.7016 5.43493 22.6583 5.39636 22.602C5.37033 22.5647 5.35224 22.5224 5.34321 22.4778C5.33418 22.4332 5.3344 22.3872 5.34386 22.3427L6.265 17.8707C5.66417 16.6909 5.3412 15.3893 5.32078 14.0655C5.30036 12.7417 5.58304 11.4308 6.14719 10.233C6.71135 9.03525 7.54202 7.98242 8.57565 7.15508C9.60928 6.32774 10.8185 5.74782 12.1107 5.45968C13.4029 5.17154 14.7439 5.18283 16.0311 5.49268C17.3183 5.80253 18.5176 6.40273 19.5372 7.24736C20.5567 8.09198 21.3695 9.15864 21.9135 10.3657C22.4574 11.5728 22.7179 12.8883 22.6752 14.2116V14.2068Z"
                            fill="white"
                        />
                    </svg>
                </div>
            </Link>
        </div>
    );
}

export default SocialIcons;