...
import { useResendResetCodeMutation, useVerifyResetOtpMutation } from '@/app/redux/service/authApis'
import { useRouter, useSearchParams } from 'next/navigation'

function ForgetPasswordOtp() {
    const router = useRouter()
    const [verifyOtp, { isLoading: verifyLoading }] = useVerifyResetOtpMutation()
    const [resendResetCode, { isLoading: resendResetCodeLoading }] = useResendResetCodeMutation()
            ...
            <CardFooter className="flex mt-3 flex-col gap-2">
                <p className="text-sm text-center text-gray-600">
                    ...
                    <span
                        ...
                        onPointerDown={handleResendCode}
                    >
                        {resendResetCodeLoading ? 'Resending...' : 'Resend OTP'}
                    </span>
                </p>
            </CardFooter>
        </div>
