...
import "./image.upload.index.css"
import Image from 'next/image';
import { X } from 'lucide-react';
import { cn } from '@/lib/utils';

function ImageUpload({ file, setFile, title, className }: { file: File | null, setFile: React.Dispatch<React.SetStateAction<File | null>>, title: string, className?: string }) {
    return (
        <div className={cn('', className)}>
            {!file ? <form className="file-upload-form">
                <h1 className='text-xl font-normal my-3'>{title}</h1>
